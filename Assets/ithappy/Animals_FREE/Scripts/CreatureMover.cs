...
namespace ithappy.Animals_FREE
{
    ...
    public class CreatureMover : MonoBehaviour
    {
        ...
        private void OnValidate()
        {
            ...
            m_Movement?.SetStats(m_WalkSpeed / 3.6f, m_RotateSpeed, m_Space);
        }
        ...
        private void Awake()
        {
            ...
            m_Movement = new MovementHandler(m_Controller, m_Transform, m_WalkSpeed, m_RotateSpeed, m_Space);
            m_Animation = new AnimationHandler(m_Animator, m_VerticalID, m_StateID);
        }

        private void Update()
        {
            m_Movement.Move(Time.deltaTime, in m_Axis, in m_Target, m_IsMoving, out var animAxis, out var isAir);
            m_Animation.Animate(in animAxis, 0f, Time.deltaTime); // chỉ đi bộ, không có trạng thái chạy
        }
        ...
        public void SetInput(in Vector2 axis, in Vector3 target, in bool _, in bool __)
        {
            m_Axis = axis;
            m_Target = target;
        ...
        private class MovementHandler
        {
            ...
            public MovementHandler(CharacterController controller, Transform transform, float walkSpeed, float rotateSpeed, Space space)
            {
            ...
            public void SetStats(float walkSpeed, float rotateSpeed, Space space)
            {
                m_WalkSpeed = walkSpeed;
                m_RotateSpeed = rotateSpeed;
                m_Space = space;
            ...
            public void Move(float deltaTime, in Vector2 axis, in Vector3 target, bool isMoving, out Vector2 animAxis, out bool isAir)
            {
            ...
            {
                Vector3 displacement = m_WalkSpeed * movement;
